...
/**
 * @brief 图书录入与修改、删除向导                                  // 未考虑藏书日期，可默认从 Get_CurrentTime 返回值赋值
 *        要求用户输入书名或ISBN
 ...
 *        如以上条件均满足，修改该 BookID 对应图书借出历史和用户借阅历史，借书时间从 Get_CurrentTime 返回值赋值（仅精确到天） // 存在跨日可能
 *                        修改该 BookID 对应图书出借状态
 *
 * @param gp
 * @param lib
...
/*!
 * @brief 按时间顺序整理函数功能：
 *          1. 要求用户输入图书书名/ISBN/ID；
 *          2. 输出图书信息：std::cout << (Book);
 *          3. 输出当前图书状态（-2：1）；
 * @param lib
 */
